...
function FilterLabel({ children }: { children: React.ReactNode }) {
  return <span className="text-xs font-medium text-[#5E6C84] dark:text-gray-400 whitespace-nowrap">{children}</span>;
}
...
function UserMultiFilter({ label, selectedValues, onChange }: UserMultiFilterProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(p => !p)}
        className={cn(btnBaseClass, hasSelection ? btnActiveClass : btnIdleClass)}
      >
        <span className="flex-1 text-left truncate">{btnLabel}</span>
        <ChevronDown size={10} className="flex-shrink-0 opacity-60" />
      </button>
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 bg-white dark:bg-gray-800 border border-[#DFE1E6] dark:border-gray-700 rounded shadow-lg z-30 min-w-[220px]">
          ...
          {hasSelection && (
            <div className="border-t border-[#DFE1E6] dark:border-gray-700 px-3 py-2">
              <button onClick={() => { onChange([]); setOpen(false); }} className="text-[11px] text-[#5E6C84] dark:text-gray-400 hover:text-red-500 transition-colors">
                Clear
              </button>
            </div>
          )}
